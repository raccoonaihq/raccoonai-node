...
```sh
npm install git+ssh://git@github.com:stainless-sdks/raccoonai-node.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainlessapi.com/docs/guides/publish), this will become: `npm install raccoonai`
...
const client = new Raccoonai({
  secretKey: process.env['SECRET_KEY'], // This is the default and can be omitted
  environment: 'environment_1', // defaults to 'production'
});

async function main() {
  const run = await client.lam.run.create({ raccoon_passcode: '<end-user-raccoon-passcode>' });
...
async function main() {
  const params: Raccoonai.Lam.RunCreateParams = { raccoon_passcode: '<end-user-raccoon-passcode>' };
...
async function main() {
  const run = await client.lam.run
    .create({ raccoon_passcode: '<end-user-raccoon-passcode>' })
    .catch(async (err) => {
...
await client.lam.run.create({ raccoon_passcode: '<end-user-raccoon-passcode>' }, {
  maxRetries: 5,
});
```
...
This can also be useful if you are getting the wrong TypeScript types for `Response` ([more details](https://github.com/stainless-sdks/raccoonai-node/tree/main/src/_shims#readme)).
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/raccoonai-node/issues) with questions, bugs, or suggestions.
